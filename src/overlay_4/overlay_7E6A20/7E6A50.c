#include "common.h"
#include "actor.h"

// "overlay 4" code for "Counter-attack", "Clance War II", "Bee's the One", and several unused scenes

extern s16 D_800D2924;
extern void func_80064AA0(s32 arg0, void* arg1);


typedef struct {
    /* 0x00 */ u16 unk_00;
    /* 0x02 */ u8 unk_02[0x2];
    /* 0x04 */ void* unk_04;
    /* 0x08 */ void* unk_08;
    /* 0x0C */ s32 unk_0C;
    /* 0x10 */ u16 unk_10;
    /* 0x12 */ u16 unk_12;
} UnkStruct_801BA6B0; /* size = 0x14 */

extern UnkStruct_801BA6B0 D_801BA6B0_7E7800[];

u16 D_801BA540_7E7690[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0090, 0x0990, 0x0253, 0x0000, 0x0000, 0x000D,
    0x0000, 0x0091, 0x0990, 0x0253, 0x0200, 0x0000, 0x000D,
    0x0000, 0x0092, 0x0000, 0x0000, 0x0000, 0x0000, 0x005E,
    SPAWNRECORD_END
};

u16 D_801BA56C_7E76BC[] = {
    0x084D, 0x088F, 0x08CF, 0x088D, 0x1111, 0x08D1, 0x1151, 0x1953,
    0x10CF, 0x1995, 0x1993, 0x0911, 0x09D3, 0x1215, 0x0A15, 0x1A15,
    0x1193, 0x1213, 0x1A19, 0x0993, 0x11D3, 0x0A13, 0x11D5, 0x0191,
    0x0A53, 0x09D1, 0x19D5, 0x09D5, 0x1255, 0x1253, 0x0991, 0x0193,
    0x0A55, 0x1153, 0x0151, 0x014F, 0x0A19, 0x125B, 0x1A9F, 0x129F,
    0x1AA1, 0x1B25, 0x1B21, 0x1B69, 0x2369, 0x23ED, 0x242D, 0x0A1B,
    0x1A5B, 0x1B65, 0x34B7, 0x34BB, 0x2CBB, 0x2C37, 0x2431, 0x23E9,
    0x0A5B, 0x1259, 0x231F, 0x2CB7, 0x2CB1, 0x2C31, 0x3D37, 0x3537,
    0x1A59, 0x2BE9, 0x3D3B, 0x45BB, 0x463B, 0x23F1, 0x125F, 0x2365,
    0x353F, 0x3DB7, 0x4637, 0x353B, 0x3DBF, 0x45B7, 0x4E3B, 0x1BE9,
    0x4E37, 0x563B, 0x1219, 0x56FB, 0x129B, 0x3DBB, 0x4EFF, 0x46FF,
    0x4EFB, 0x463F, 0x2321, 0x12A1, 0x2D3B, 0x0887, 0x0889, 0x110B,
    0x29D1, 0x214D, 0x298F, 0x3A13, 0x10C9, 0xFFFF, 0, 0
};



u16 D_801BA63C_7E778C[] = {
    0x1000, 0x027F, 0x0000, 0x7FFF, 0x0574, 0x020F, 0xFFE0, 0xFFEB
};

// unusewd spawn table
u16 D_801BA64C_7E779C[] ={
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0048, 0x1000, 0x01B0, 0x0000, 0x0000, 0x1F01,
    SPAWNRECORD_END,
};

u16 D_801BA65C_7E77AC[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0030, 0x0000, 0x0000, 0x0000, 0x0002, ACTORTYPE_110,
    SPAWNRECORD_END,
};

u16 D_801BA66C_7E77BC[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0030, 0x1000, 0x0290, 0x0000, 0x0000, 0x1F01,
    0x0000, 0x0050, 0x1100, 0x0290, 0x0000, 0x0000, 0x1F03,
    SPAWNRECORD_END,
    0, 0, 0, // might be a seperate unused table
    0x0000, 0x0030, 0x1000, 0x0290, 0x0000, 0x0000, 0x1F01,
    SPAWNRECORD_END
};

u32 D_801BA6A0_7E77F0[] = {
    0x0200027F, 0x00003100, 0x02EF020F, 0xFFE0FFEB,
};

UnkStruct_801BA6B0 D_801BA6B0_7E7800[] = {
    { 0x0600, { 0 }, D_801BA704_7E7854, D_801BA714_7E7864, (s32)D_801BA700_7E7850, 0x0200, 0x0800 },
    { 0x1100, { 0 }, D_801BA724_7E7874, D_801BA734_7E7884, (s32)D_801BA700_7E7850, 0x0800, 0x1300 },
    { 0x1E00, { 0 }, D_801BA744_7E7894, D_801BA754_7E78A4, (s32)D_801BA700_7E7850, 0x1300, 0x2000 },
    { 0x2C00, { 0 }, D_801BA764_7E78B4, D_801BA774_7E78C4, (s32)D_801BA700_7E7850, 0x2000, 0x2E00 },
};

u32 D_801BA700_7E7850[] = {
    0x00000000,
};

u16 D_801BA704_7E7854[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0031, 0x0000, 0x0000, 0x0000, 0x0000, 0x2900,
    SPAWNRECORD_END
};

u16 D_801BA714_7E7864[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0080, 0x07C0, 0x0260, 0x0001, 0x0060, 0x2906,
    SPAWNRECORD_END
};

u16 D_801BA724_7E7874[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0031, 0x0000, 0x0000, 0x0000, 0x0001, 0x2900,
    SPAWNRECORD_END
};

u16 D_801BA734_7E7884[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0080, 0x12C0, 0x0260, 0x2101, 0x0060, 0x2906,
    SPAWNRECORD_END
};

u16 D_801BA744_7E7894[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0031, 0x0000, 0x0000, 0x0000, 0x0002, 0x2900, 
    SPAWNRECORD_END
};

u16 D_801BA754_7E78A4[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0080, 0x1FC0, 0x0260, 0x2201, 0x0060, 0x2906,
    SPAWNRECORD_END
};

u16 D_801BA764_7E78B4[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0031, 0x0000, 0x0000, 0x0000, 0x0003, 0x2900,
    SPAWNRECORD_END
};

u16 D_801BA774_7E78C4[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0080, 0x2DC0, 0x0260, 0x2301, 0x0060, 0x2906, 
    SPAWNRECORD_END
};

u16 D_801BA784_7E78D4[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0030, 0x2DC0, 0x02A0, 0x0081, 0x0000, 0x0032,
    SPAWNRECORD_END
};

u16 D_801BA794_7E78E4[] = {
    0x01A0, 0x027F, 0x0100, 0x19A0, 0x0574, 0x020F, 0xFFE0, 0xFFEB,
};

u16 D_801BA7A4_7E78F4[] = {
    0x0005, 0xF7BD, 0x6319, 0x5AD7, 0x4A53, 0x4211, 0x39CF, 0x318D,
    0x294B, 0x2109, 0x18C7, 0x1085, 0x0843, 0xFFBD, 0xF77B, 0xDEB5,
    0xCE31, 0xC5EF, 0xD631, 0xCDEF, 0xC5AD, 0xBD6B, 0xB529, 0x5A95,
    0xA4A5, 0xE673, 0x9421, 0x7B5B, 0x41CF, 0xB4E7, 0xA463, 0x9C21,
    0x314B, 0x93DF, 0x8B9D, 0x5A53, 0x835B, 0x7B19, 0x5211, 0x2909,
    0x72D7, 0x49CF, 0x6A95, 0x6253, 0x418D, 0x9B9D, 0x5A11, 0x20C7,
    0x8B19, 0x51CF, 0x3109, 0x1885, 0x498D, 0x59CF, 0x414B, 0x8A95,
    0x3909, 0x61CF, 0x7211, 0x9295, 0x494B, 0x598D, 0x4109, 0x1043,
    0x71CF, 0x30C7, 0x2085, 0x38C7, 0x2885, 0x40C7, 0x1843, 0x48C7,
    0x50C7, 0x4085, 0x2043, 0x68C7, 0x4885, 0x78C7, 0x80C7, 0x90C7,
    0x6085, 0xA0C7, 0x7085, 0x7885, 0x8085, 0x4043, 0x9085, 0x5843,
    0x7043, 0x2801, 0x2001, 0x1801, 0x1001, 0x58C5, 0x5083, 0x5883,
    0x3841, 0x3041, 0x4907, 0x2841, 0x30C5, 0x2883, 0x28C5, 0x2083,
    0x3949, 0x3107, 0x4189, 0x20C5, 0x30C1, 0x398B, 0x3947, 0x1883,
    0x3903, 0x628F, 0x3103, 0x20C3, 0x5A91, 0x3989, 0xB567, 0x62D1,
    0xBDA7, 0x9C9F, 0x5A8F, 0x5A8D, 0xEF79, 0xB5AB, 0x7BDD, 0xDEF3,
    0xD6B1, 0xC62D, 0x5293, 0xA525, 0x9CE3, 0x7BDB, 0x420F, 0x949F,
    0x6315, 0x8C5D, 0xAD63, 0xA521, 0x7397, 0x949D, 0x8419, 0x6313,
    0x420D, 0x7BD7, 0x7395, 0x6B53, 0x528F, 0x18C5, 0x4A4D, 0x5ACF,
    0x420B, 0x4A4B, 0x6B4F, 0x528B, 0x1083, 0x630D, 0x3187, 0x2105,
    0x424D, 0x10C5, 0x10C7, 0x0043, 0x110B, 0x110D, 0x10C9, 0x10CB,
    0x2993, 0x8423, 0x6B5D, 0x4213, 0x294D, 0x4A57, 0x4215, 0x39D3,
    0x3191, 0x4217, 0x294F, 0x4A5B, 0x39D5, 0x210D, 0x529F, 0x4219,
    0x2951, 0x18CB, 0x18CD, 0x1089, 0x108B, 0x0849, 0x0007, 0x0005,
    0x0003, 0x188D, 0x290F, 0x188B, 0x1049, 0x41D5, 0x20CB, 0x1849,
    0x28CD, 0x208B, 0x1007, 0x20C9, 0x310D, 0x2089, 0x288B, 0x1847,
    0x2049, 0x1005, 0x30CB, 0x5A15, 0x390D, 0x414F, 0x2047, 0xD633,
    0x5213, 0x394D, 0x38CB, 0x1845, 0x3089, 0x1003, 0xC5AF, 0x6215,
    0x3007, 0x2005, 0x40CB, 0x72D9, 0x51D1, 0x30C9, 0x7B1B, 0x5A13,
    0x390B, 0x835D, 0x6A97, 0x6255, 0x2887, 0x2045, 0x3847, 0x3005,
    0x4889, 0x59D1, 0x939F, 0x831B, 0x7297, 0x2845, 0x4005, 0x2003,
    0x48C9, 0x6889, 0x4845, 0x5845, 0x6887, 0x8045, 0x0001, 0xF800,
};

u16 D_801BA9A4_7E7AF4[] = {
    0x0050, 0x0300, 0x02B0, 0x0050, 0x0330, 0x0290, 0x0050, 0x0360,
    0x02B0, 0x0051, 0x03C0, 0x02E0, 0x0051, 0x03F0, 0x0300, 0x0051,
    0x0420, 0x02E0, 0x0051, 0x16C0, 0x04B0, 0x0051, 0x16F0, 0x0480,
    0x0051, 0x16F0, 0x04B0, 0x0051, 0x16F0, 0x04E0, 0x0051, 0x1720,
    0x04B0, 0x0050, 0x17E0, 0x04B0, 0x0050, 0x1810, 0x0480, 0x0050,
    0x1810, 0x04B0, 0x0050, 0x1810, 0x04E0, 0x0050, 0x1840, 0x04B0,
    0, 0
};

u16 D_801BAA08_7E7B58[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0030, 0x01A0, 0x0290, 0x0000, 0x0000, 0x1F02,
    0x0000, 0x0070, 0x0200, 0x02A0, 0x0000, 0x0000, 0x290F,
    0x0000, 0x0092, 0x0200, 0x02A0, 0x0000, 0x0000, 0x2912,
    0x0000, 0x0093, 0x0200, 0x02A0, 0x0000, 0x0001, 0x2912,
    0x0000, 0x004A, 0x05C0, 0x0300, 0x0211, 0x0000, 0x2911,
    0x0000, 0x004B, 0x0680, 0x0318, 0x0311, 0x0000, 0x2911,
    0x0000, 0x004C, 0x0740, 0x0330, 0x0411, 0x0000, 0x2911,
    0x0000, 0x004D, 0x0800, 0x0348, 0x0511, 0x0000, 0x2911,
    0x0000, 0x004E, 0x08C0, 0x0360, 0x0611, 0x0000, 0x2911,
    0x0000, 0x004F, 0x0980, 0x0378, 0x0711, 0x0000, 0x2911,
    0x0000, 0x0050, 0x0A40, 0x0390, 0x0011, 0x0000, 0x2911,
    0x0000, 0x0051, 0x0B00, 0x03A8, 0x0111, 0x0000, 0x2911,
    0x0000, 0x0052, 0x0BC0, 0x03C0, 0x0211, 0x0000, 0x2911,
    0x0000, 0x0053, 0x0C80, 0x03D8, 0x0311, 0x0000, 0x2911,
    0x0000, 0x0054, 0x0D40, 0x03F0, 0x0411, 0x0000, 0x2911,
    0x0000, 0x0055, 0x0E00, 0x0408, 0x0511, 0x0000, 0x2911,
    0x0000, 0x0056, 0x0EC0, 0x0420, 0x0611, 0x0000, 0x2911,
    0x0000, 0x0057, 0x0F80, 0x0438, 0x0711, 0x0000, 0x2911,
    0x0000, 0x0058, 0x1040, 0x0450, 0x0011, 0x0000, 0x2911,
    0x0000, 0x0059, 0x1100, 0x0468, 0x0111, 0x0000, 0x2911,
    0x0000, 0x005A, 0x11C0, 0x0480, 0x0211, 0x0000, 0x2911,
    0x0000, 0x005B, 0x1280, 0x0498, 0x0311, 0x0000, 0x2911,
    0x0000, 0x005C, 0x1340, 0x04B0, 0x0411, 0x0000, 0x2911, 
    0x0000, 0x005D, 0x1400, 0x04C8, 0x0511, 0x0000, 0x2911,
    0x0000, 0x005E, 0x14C0, 0x04E0, 0x0611, 0x0000, 0x2911,
    0x0000, 0x005F, 0x1580, 0x04C8, 0x0711, 0x0000, 0x2911,
    SPAWNRECORD_END,
};

u16 D_801BAB78_7E7CC8[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x0060, 0x16F0, 0x04B0, 0x0100, 0x0004, ACTORTYPE_76,
    0x0000, 0x0061, 0x16F0, 0x04B0, 0x0200, 0x0005, ACTORTYPE_76, 
    0x0000, 0x0062, 0x16F0, 0x04B0, 0x0000, 0x0006, ACTORTYPE_76,
    0x0000, 0x0063, 0x16F0, 0x04B0, 0x0100, 0x0007, ACTORTYPE_76,
    0x0000, 0x0064, 0x16F0, 0x04B0, 0x0200, 0x0008, ACTORTYPE_76,
    0x0000, 0x0065, 0x16F0, 0x04B0, 0x0000, 0x0009, ACTORTYPE_76,
    0x0000, 0x0066, 0x16F0, 0x04B0, 0x0100, 0x000A, ACTORTYPE_76,
    0x0000, 0x0067, 0x16F0, 0x04B0, 0x0200, 0x000B, ACTORTYPE_76,
    0x0000, 0x0068, 0x1810, 0x04B0, 0x0100, 0x000C, ACTORTYPE_76,
    0x0000, 0x0069, 0x1810, 0x04B0, 0x0200, 0x000D, ACTORTYPE_76,
    0x0000, 0x006A, 0x1810, 0x04B0, 0x0000, 0x000E, ACTORTYPE_76,
    0x0000, 0x006B, 0x1810, 0x04B0, 0x0100, 0x000F, ACTORTYPE_76,
    0x0000, 0x006C, 0x1810, 0x04B0, 0x0200, 0x0010, ACTORTYPE_76,
    0x0000, 0x006D, 0x1810, 0x04B0, 0x0000, 0x0011, ACTORTYPE_76,
    0x0000, 0x006E, 0x1810, 0x04B0, 0x0100, 0x0012, ACTORTYPE_76,
    0x0000, 0x006F, 0x1810, 0x04B0, 0x0200, 0x0013, ACTORTYPE_76,
    0x0000, 0x0088, 0x1910, 0x0428, 0x0014, 0x0003, 0x291A,
    0x0000, 0x0089, 0x1910, 0x0428, 0x0015, 0x0004, 0x291A,
    0x0000, 0x008A, 0x1910, 0x0428, 0x0014, 0x0005, 0x291A,
    0x0000, 0x008B, 0x1970, 0x0458, 0x0000, 0x0000, 0x291A,
    0x0000, 0x008C, 0x1970, 0x0458, 0x0009, 0x0000, 0x291A,
    0x0000, 0x008D, 0x1A30, 0x0458, 0x0000, 0x0000, 0x291A,
    0x0000, 0x008E, 0x1A30, 0x0458, 0x0009, 0x0000, 0x291A, 
    0x0000, 0x008F, 0x19A0, 0x04D0, 0x0081, 0x0000, ACTORTYPE_WARPGATE,
    SPAWNRECORD_END
};

u16 D_801BACCC_7E7E1C[] = {
    0x1000, 0x019C, 0x0000, 0x7FFF, 0x020C, 0x012C, 0xFFE0, 0xFFEB,
};

u16 D_801BACDC_7E7E2C[] = {
    0,0
};

u16 D_801BACE0_7E7E30[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x0000, 0x008F, 0x1000, 0x01B0, 0x0001, 0x0000, ACTORTYPE_WARPGATE,
    SPAWNRECORD_END
};

u16 D_801BACF0_7E7E40[] = {
    0x1000, 0x019C, 0x0000, 0x7FFF, 0x020C, 0x012C, 0xFFE0, 0xFFEB,
};

u16 D_801BAD00_7E7E50[] = {
    0,0
};

u16 D_801BAD04_7E7E54[] = {
//  flags   index   posX    posY    0x110   0xD8    type
    0x000, 0x0008F, 0x1000, 0x01B0, 0x0001, 0x0000, ACTORTYPE_WARPGATE, 
    SPAWNRECORD_END
};

void func_801B9900_7E6A50(u16 actor_index);
void func_801B996C_7E6ABC(void);
void func_801B99F4_7E6B44(u16 arg0);
void func_801B9A08_7E6B58(void);
s32 func_801B9DAC_7E6EFC(void);
void func_801B9E7C_7E6FCC(void);
void func_801B9F50_7E70A0(void);
void func_801B9FB4_7E7104(void);
void func_801B9FC8_7E7118(void);
void func_801BA1DC_7E732C(void);
void func_801BA3C4_7E7514(void);
void func_801BA480_7E75D0(void);

void func_801B9900_7E6A50(u16 actor_index) {
    gActors[actor_index].graphicFlags |= ACTOR_GFLAG_PALETTE;
    gActors[actor_index].palette_18C = D_801BA56C_7E76BC;
    Actor_SetColorRgb(actor_index, 8);
}

void func_801B996C_7E6ABC(void) {
    s32 temp;

    if ((gActors[0x90].actorType == ACTORTYPE_13) && ((gActors[0x90].flags & ACTOR_FLAG_ACTIVE) != 0)) {
        func_801B9900_7E6A50(0x90);
        func_801B9900_7E6A50(0x91);
    }

    temp = (s16)func_800456DC();
    D_801BA56C_7E76BC[101] = Palette_AdjustRgb5551(0x47DF, temp, temp, temp);
}

void func_801B99F4_7E6B44(u16 arg0) {
    D_800BE73C = 0x60000;
}

void func_801B9A08_7E6B58(void) {
    if ((gButtonPress & gButton_LTrig) != 0) {
        D_800CC428 = 1;
    }

    switch (gStageCinemaState) {
        case 0:
            gStageCinemaState = 0x100;
            break;

        case 0x100:
            gStageCinemaState = 0x300;
            break;

        case 0x101:
            if (func_80046D5C() != 0) {
                gStageCinemaState++;
                D_800D28FC |= 8;
                D_800BE5F4.unk_00_u32 = 5;
                D_800D2928 = 60;
                Sound_StartFade(0x81, 0x37);
            }
            break;

        case 0x102:
            func_801B99F4_7E6B44(0x30);
            D_800D2928--;
            if (D_800D2928 < 0) {
                gStageCinemaState = 0x200;
            }
            break;

        case 0x200:
            gStageCinemaState++;
            Actor_ClearSceneActors();
            func_80045FA4(D_801BA63C_7E778C, 0);
            D_800BE5F4.unk_00_u32 = 4;
            D_800BE544 = 0x8000;
            gDrawMidground = FALSE;
            gDrawEnvLayer = FALSE;
            gDrawBackground = FALSE;
            Actor_LoadSpawnTable(D_801BA65C_7E77AC);
            func_80046A9C();
            Sound_PlayMusic(BGM_BONO);

        case 0x201:
            if (func_80046D5C() != 0) {
                gStageCinemaState++;
            }
            break;

        case 0x202:
            if (((gButtonPress & gButton_ZTrig) != 0) || (gActors[0x30].flags == 0)) {
                D_800D2938 = 0;
                gStageCinemaState = 0x300;
                D_800D28F0 = D_800D28E4;
                D_800D28E4 = 0x61;
                D_800D2928 = 1;
            }
            D_800BE73C = 0;
            break;

        case 0x300:
            gStageCinemaState++;
            func_80045FA4(D_801BA63C_7E778C, 0);
            Actor_LoadSpawnTable(D_801BA540_7E7690);
            Actor_LoadSpawnTable(D_801BA66C_7E77BC);
            gLifebar.posY.whole = -0x58;
            gLifebarHead.posY.whole = -0x55;
            gDrawMidground = TRUE;
            gDrawBackground = TRUE;
            func_801B99F4_7E6B44(0x30);

        case 0x301:
            if (func_80046D5C() != 0) {
                gStageCinemaState++;
                D_800D28FC |= 8;
                D_800BE5F4.unk_00_u32 = 5;
            }
            break;

        case 0x302:
            if (gActors[0x50].flags == 0) {
                gStageCinemaState++;
            }
            func_801B99F4_7E6B44(0x30);
            break;

        case 0x303:
            gStageCinemaState++;
            D_800D2928 = 0x78;

        case 0x304:
            D_800D2928--;
            if (D_800D2928 < 0) {
                D_800D28E4 = 0x64;
            }
            func_801B99F4_7E6B44(0x30);
            break;
    }

    func_801B996C_7E6ABC();
}

s32 func_801B9DAC_7E6EFC(void) {
    if (D_801BA6B0_7E7800[D_800D2928].unk_10 < gScreenPosCurrentX.whole) {
        gActors[0x31].var_15C |= 2;
    }

    if (D_801BA6B0_7E7800[D_800D2928].unk_00 < gScreenPosCurrentX.whole) {
        Actor_LoadSpawnTable(D_801BA6B0_7E7800[D_800D2928].unk_08);
        D_800BE568.whole  = D_801BA6B0_7E7800[D_800D2928].unk_00 - 0x90;
        gActors[0x31].var_15C |= 1;
        return TRUE;
    }

    return FALSE;
}

void func_801B9E7C_7E6FCC(void) {
    UnkStruct_801BA6B0* entry;
    s32 temp;

    if (D_800D2928 < 4) {
        func_80045FA4(D_801BA6A0_7E77F0, (u16*)D_801BA6B0_7E7800[D_800D2928].unk_0C);

        entry = &D_801BA6B0_7E7800[D_800D2928];
        temp = entry->unk_10;
        D_800BE568.whole  = temp - 0x90;
        D_800BE56C.whole = entry->unk_12 + 0x90;
        D_800BE570.whole  = 0x2EF;
        D_800BE574.whole = 0x20F;
        gScreenPosCurrentX.whole = temp + 0x90;
        gScreenPosCurrentY.whole = 0x27F;
        gMarina.posX.whole = -0x90;
        gMarina.posY.whole = -0x15;
    }
}

void func_801B9F50_7E70A0(void) {
    u16 temp_s0;
    u16 temp_s1;

    temp_s0 = D_800BE568.whole ;
    temp_s1 = D_800BE56C.whole ;
    func_801B9E7C_7E6FCC();
    Actor_LoadSpawnTable(D_801BA540_7E7690);
    D_800D292C = D_800BE56C.whole ;
    D_800BE568.whole  = temp_s0;
    D_800BE56C.whole  = temp_s1;
}

void func_801B9FB4_7E7104(void) {
    D_800BE56C.whole  = D_800D292C;
}

void func_801B9FC8_7E7118(void) {
    switch (gStageCinemaState) {
        case 0:
            D_800D2928 = 0;
            Palette_AdjustScenePalettes(1, 2, 0, 0, 1, 2, 2, 1, 0);
            func_801B9E7C_7E6FCC();
            Actor_LoadSpawnTable(D_801BA540_7E7690);
            Actor_LoadSpawnTable(D_801BA704_7E7854);
            gStageCinemaState++;
            D_800D2930 = 0;

        case 1:
            if (func_80046D5C() != 0) {
                gStageCinemaState++;
                D_800D28FC |= 8;
            }
            break;

        case 2:
            if (func_801B9DAC_7E6EFC() != 0) {
                gStageCinemaState++;
            }
            break;

        case 3:
            if (gActors[0x80].flags == 0) {
                gStageCinemaState++;
                Actor_ClearSceneActors();
                func_8003D628(0);
                D_800D2928++;
                D_800D2930 = 0xA;
            }
            break;

        case 4:
            D_800D2930--;
            if (D_800D2930 <= 0) {
                gStageCinemaState++;
                func_801B9F50_7E70A0();
                return;
            }
            break;

        case 6:
            func_801B9FB4_7E7104();
            if (D_800D2928 >= 4) {
                gStageCinemaState++;
                func_80064AA0(1, D_801BA784_7E78D4);
                return;
            }
            gStageCinemaState = 1;
            Actor_LoadSpawnTable(D_801BA6B0_7E7800[D_800D2928].unk_04);
            break;

        case 5:
        case 7:
            break;
    }
}

void func_801BA1DC_7E732C(void) {
    u16 index;
    u16* base;
    u16* src16;
    u16* dst16;
    u32* src;
    u32* dst;

    switch (gStageCinemaState) {
        case 0:
            base = PALETTE_80380400;
            src = (u32*)base;
            dst16 = base;
            src16 = D_801BA7A4_7E78F4;
            for (index = 0; index < 0x100; index++) {
                *dst16++ = *src16++;
            }

            dst = (u32*)PALETTE_803DA600;
            index = 0;
            src = (u32*)base;
            for (; index < 0x80; index++) {
                *dst++ = *src++;
            }

            Palette_AdjustScenePalettes(1, 2, 0, 0, 1, 2, 2, 1, 0);
            Palette_AdjustRgb5551Array((void*)0x802F8068, (void*)0x802F7E68, 0xFF, -2, -3, -4);
            func_80045FA4(D_801BA794_7E78E4, D_801BA9A4_7E7AF4);
            if (!D_801BA7A4_7E78F4) {
            }
            D_800D2924 = 0x2000;
            Actor_LoadSpawnTable(D_801BA540_7E7690);
            Actor_LoadSpawnTable(D_801BAA08_7E7B58);
            gStageCinemaState++;

        case 1:
            if (func_80046D5C() != 0) {
                D_800D28FC |= 8;
                D_800BE5F4.unk_00_u32 = 5;
                gStageCinemaState++;
            }
            break;

        case 2:
            if (gScreenPosCurrentX.whole > 0x1600) {
                Actor_LoadSpawnTable(D_801BAB78_7E7CC8);
                gStageCinemaState++;
            }
            break;

        case 3:
            break;
    }
}

void func_801BA3C4_7E7514(void) {
    switch (gStageCinemaState) {
        case 0:
            gStageCinemaState++;
            func_80045FA4(D_801BACCC_7E7E1C, D_801BACDC_7E7E2C);
            Actor_LoadSpawnTable(D_801BA540_7E7690);
            Actor_LoadSpawnTable(D_801BACE0_7E7E30);

        case 1:
            if (func_80046D5C() != 0) {
                gStageCinemaState++;
                D_800D28FC |= 8;
                D_800BE5F4.unk_00_u32 = 5;
            }
            break;

        case 2:
            break;
    }
}

void func_801BA480_7E75D0(void) {
    switch (gStageCinemaState) {
        case 0:
            gStageCinemaState++;
            func_80045FA4(D_801BACF0_7E7E40, D_801BAD00_7E7E50);
            Actor_LoadSpawnTable(D_801BA540_7E7690);
            Actor_LoadSpawnTable(D_801BAD04_7E7E54);

        case 1:
            if (func_80046D5C() != 0) {
                gStageCinemaState++;
                D_800D28FC |= 8;
                D_800BE5F4.unk_00_u32 = 5;
            }
            break;

        case 2:
            break;
    }
}
